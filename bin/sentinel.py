...
def main():
    ...
    if not is_wantd_port_open(wantd):
        print("Cannot connect to wantd. Please ensure wantd is running and the JSONRPC port is open to Sentinel.")
        return

    # check wantd sync
    if not wantd.is_synced():
        print("wantd not synced with network! Awaiting full sync before running Sentinel.")
        return
